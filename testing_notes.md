...
However, note this can no longer be easily repeated, because we chose to remove the TEST preprocessing segments to simplify code appearance.  Now small tabulations would require changing the hardcoded values of B and X.
...
Timings in seconds, with the new codebase, serial timings, early abort flag turned off or on
